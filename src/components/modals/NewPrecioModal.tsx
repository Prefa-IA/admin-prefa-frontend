    ...
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-md space-y-4 shadow-xl">
        ...
        <div className="space-y-2">
          <label className="block text-sm font-medium">Barrio
            <input className="input-field w-full" value={barrio} onChange={e => setBarrio(e.target.value)} />
          </label>
          <label className="block text-sm font-medium">Valor USD
            <input type="number" className="input-field w-full" value={valor} onChange={e => setValor(e.target.value === '' ? '' : Number(e.target.value))} />
          </label>
        </div>
