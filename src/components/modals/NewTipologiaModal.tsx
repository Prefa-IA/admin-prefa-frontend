...
const NewTipologiaModal: React.FC<Props> = ({ onClose, onSave }) => {
  ...
  const handleSubmit = () => {
    ...
    onSave({
      ...
      alturaMin: Number(alturaMin),
      alturaMax: Number(alturaMax),
    });
  };
        ...
        <div className="space-y-2">
          ...
          <label className="block text-sm font-medium">
            ...
            <input
              ...
              className="input-field w-full"
              value={alturaMin}
              onChange={(e) => setAlturaMin(e.target.value === '' ? '' : Number(e.target.value))}
            />
          </label>
          <label className="block text-sm font-medium">
            Altura máxima (m)
            <input
              ...
              className="input-field w-full"
              value={alturaMax}
              onChange={(e) => setAlturaMax(e.target.value === '' ? '' : Number(e.target.value))}
            />
          </label>
        </div>
